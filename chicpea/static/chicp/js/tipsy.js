...
(function($) {
    ...
    $.fn.tipsy.autoNSEW = function() {
    	var totalHeight = $($(this)[0].nearestViewportElement).height();
    	var totalWidth = $($(this)[0].nearestViewportElement).width();
    	...
    	if (position < totalHeight/4) return 's';
    	if (position > totalHeight/4 && position < 3*totalHeight/4){
    		position2 = $(this).position().left
    		position2 += rect.width/2;
    		if (position2 < totalWidth/2) return 'e';
    		return 'w'
    	}
    	if (position > 3*totalHeight/4) return 'n';
